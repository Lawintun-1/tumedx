...
`;
    ...
    form.addEventListener('submit', (e) => {
        ...
        chapterCards.forEach(chapterCard => {
            ...
            // Only process chapters with titles
            ...
            chapterCard.querySelectorAll('.session-card').forEach(sessionCard => {
                ...
                const sessionTitle = sessionCard.querySelector('.learning-session-title-input')?.value || 
                                   sessionCard.querySelector('.quiz-name-input')?.value;

                // Only process sessions with titles
                ...
                if (sessionType === 'learning') {
                    ...
                    sessionCard.querySelectorAll('.material-form-container').forEach(materialCard => {
                        ...
                        // Get content from the active field
                        let materialContent = '';
                        const activeField = materialCard.querySelector(`.material-content-field[data-type="${materialType}"]:not(.hidden)`);
                        
                        if (activeField) {
                            if (materialType === 'text') {
                                materialContent = activeField.querySelector('textarea').value;
                            } else {
                                materialContent = activeField.querySelector('input[type="url"]').value;
                            }
                        }

                        // Only add materials with both name and content
                    ...
                    // Only add session if it has materials or we want to allow empty sessions
                    ...
                    const quizSessionData = {
                        type: 'quiz_session',
                        quiz_name: sessionTitle,
                        content_source_type: quizGeneratorCard.dataset.contentSourceType || 'text_input',
                        ai_content: quizGeneratorCard.dataset.aiContent || '',
                        ai_file_name: quizGeneratorCard.dataset.aiFileName || '',
                        question_type: quizGeneratorCard.querySelector('.question-type-select')?.value || 'mcq',
                        bloom_level: quizGeneratorCard.querySelector('.bloom-level-select')?.value || 'remember',
                        question_difficulty: quizGeneratorCard.dataset.questionDifficulty || 'medium',
                        num_questions: quizGeneratorCard.dataset.numQuestions || '3',
        ...
        console.log('Course content to submit:', allCourseContent);
