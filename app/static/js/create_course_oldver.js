...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chaptersContainer.addEventListener('click', (e) => {
        ...
        // Handle content source type change within quiz generator (delegated, this is a redundant listener due to the one above)
        // This block is slightly redundant if the change listener is added directly upon session creation,
        // but it doesn't hurt to keep it if it's meant to catch events for pre-existing elements.
        // For new elements, the listener in 'Add Quiz Session' block is primary.
        ...
        if (e.target.closest('.generate-questions-btn')) {
            ...
            if (contentSourceType === 'text_input') {
                ...
                if (fileInput.files.length > 0) {
                    aiFile = fileInput.files[0];
                    aiFileName = aiFile.name;
                    // For actual API call, the file itself needs to be sent via FormData
                    // For now, we'll just capture its name.
                } else {
            ...
            const questionDifficulty = quizGeneratorCard.querySelector('.question-difficulty-select').value; // New
            const numQuestions = quizGeneratorCard.querySelector('.num-questions-input').value; // New
            ...
            console.log("Generating questions with:", { aiContent: aiContent || aiFileName, contentSourceType, questionType, bloomLevel, questionDifficulty, numQuestions });
            ...
            setTimeout(() => {
                ...
                for (let i = 0; i < parsedNumQuestions; i++) {
                    if (questionType === 'mcq') {
                        sampleQuestions.push(
                            { text: `MCQ Q${i+1}: What is X based on the content? (${bloomLevel}, ${questionDifficulty})`, type: 'mcq', choices: [{ text: 'Choice A', is_correct: false }, { text: 'Choice B', is_correct: true }, { text: 'Choice C', is_correct: false }], correct_answer: 'Choice B' }
                        );
                    } else if (questionType === 'true_false') {
                        sampleQuestions.push(
                            { text: `T/F Q${i+1}: Statement Y is true. (${bloomLevel}, ${questionDifficulty})`, type: 'true_false', correct_answer: 'True' }
                        );
                    } else if (questionType === 'fill_blank') {
                        sampleQuestions.push(
                            { text: `FIB Q${i+1}: The main concept is ________. (${bloomLevel}, ${questionDifficulty})`, type: 'fill_blank', correct_answer: 'Concept' }
                        );
                    }
                }
                ...
                // Store generated questions and new parameters in a data attribute for later serialization
                ...
                if (contentSourceType === 'text_input') {
                    quizGeneratorCard.dataset.aiContent = aiContent;
                } else {
                    quizGeneratorCard.dataset.aiFileName = aiFileName;
                    // Handle actual file upload via separate AJAX if needed for saving
            ...
            chapterCard.querySelectorAll('.session-card').forEach(sessionCard => {
                ...
                if (sessionType === 'learning') {
                    ...
                    sessionCard.querySelectorAll('.material-form-container').forEach(materialCard => {
                        ...
                        let materialContent = ''; // Initialize to empty string

                        // --- FIX APPLIED HERE ---
                        // Dynamically find the active content input based on its data-type
                        ...
                        if (activeContentField) {
                            ...
                            if (inputElement) {
                                materialContent = inputElement.value; // Assign the value from the active input
                            }
                        }
                        // --- END FIX ---
                        ...
                        learningSessionData.materials.push({
                            type: materialType,
                            name: materialName,
                            content: materialContent, // This will now correctly capture the URL or text
                        });
                    });
